...
def verify_same_geometry(img_1: sitk.Image, img_2: sitk.Image):
    ...
    same_ori = np.all(np.isclose(ori1, ori2, rtol=1e-3, atol=1e-3))
    if not same_ori:
    ...
    same_dir = np.all(np.isclose(direction1, direction2, rtol=1e-3))
    if not same_dir:
        print("the direction does not match between the images")
        print(direction1)
        print(direction2)

    same_size = np.all(np.isclose(size1, size2, rtol=1e-3,))
    if not same_size:
...
def verify_dataset_integrity(folder):
    ...
    if folder.split('/')[-1].startswith('Task128'):

        expected_labels = [0] + list(int(i) for i in dataset['labels'].keys())
    else:
    ...
    if has_nan:
        ...
        #be careful here (changed from nnUNet default)
        print("Some images have nan values in them. This will break the training. See text output above to see which ones")
